...
import { UserPlus, BookOpen, Video } from 'lucide-react';
...
const mockRequests = [
  { id: 'INT-201', requester: 'Oarabile T.', email: 'oarabile@example.com', service_type: 'Script Only', requested_at: new Date().toISOString(), status: 'pending', partner: null, pref_date: null },
  { id: 'INT-202', requester: 'Mpho K.', email: 'mpho@example.com', service_type: 'Virtual + Script', requested_at: new Date(Date.now() - 86400000).toISOString(), status: 'assigned', partner: 'HR Experts BW', pref_date: new Date(Date.now() + 86400000 * 3).toISOString() },
];

const mockPartners = ['HR Experts BW', 'Gaborone Resumes', 'Interview Coach Pro'];

export const InterviewPrepAdmin = () => {
  ...
  const getStatusBadge = (status) => {
    const map = { pending: 'warning', assigned: 'info', completed: 'success' };
  ...
  return (
    <div className="p-6 flex flex-col gap-6 max-w-7xl">
      <div>
        <h1 className="text-xl font-display font-bold text-slate-900">Interview Prep Requests</h1>
        <p className="text-sm text-slate-500 mt-0.5">Manage interview coaching and scripting requests.</p>
      </div>
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-100 text-[10px] uppercase text-slate-500 tracking-wider">
              ...
              <th className="px-5 py-3">Pref. Date</th>
          ...
          <tbody className="divide-y divide-slate-100 text-sm text-slate-700">
            {filtered.map(req => (
              <tr key={req.id} className="hover:bg-slate-50/50 transition-colors">
                ...
                <td className="px-5 py-4">
                  <span className={`flex items-center gap-1.5 text-xs font-semibold ${req.service_type.includes('Virtual') ? 'text-purple-600' : 'text-blue-600'}`}>
                    {req.service_type.includes('Virtual') ? <Video size={14} /> : <BookOpen size={14} />} {req.service_type}
                  </span>
                </td>
                <td className="px-5 py-4 text-slate-500">{formatDate(req.requested_at)}</td>
                <td className="px-5 py-4 text-slate-600">{req.pref_date ? formatDate(req.pref_date) : '-'}</td>
            ...
            {filtered.length === 0 && (
              <tr>
                <td colSpan="8" className="px-5 py-12 text-center text-slate-500">No requests found for this filter.</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      <Modal isOpen={showAssignModal} onClose={() => setShowAssignModal(false)} title="Assign Partner">
        <div className="p-5 flex flex-col gap-4">
          <p className="text-sm text-slate-600">Assign <span className="font-bold">{selectedReq?.requester}</span> to a partner for {selectedReq?.service_type}.</p>
